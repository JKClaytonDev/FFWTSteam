...
namespace GreatArcStudios
{
    ...
    public class PauseManager : MonoBehaviour
    {
        ...
        public void Update()
        {
            ...
            //colorCrossfade();
            if (vidPanel && audioPanel && mainPanel)
            {
            ...
            if (SceneManager.GetActiveScene().name != "MainMenu" && Input.GetKeyDown(KeyCode.Escape) && mainPanel.active == false)
            {
            ...
            else if(SceneManager.GetActiveScene().name != "MainMenu" && Input.GetKeyDown(KeyCode.Escape) && mainPanel.active == true) {
                Time.timeScale = timeScale;
                mainPanel.SetActive(false);
                vidPanel.SetActive(false);
                audioPanel.SetActive(false);
        ...
        public void updateEffectsVol(float f)
        {
            try
            {
                PlayerPrefs.SetFloat("Mouse", f*3);
            }
            catch
            {
        ...
        internal IEnumerator cancelVideoMain()
        {
            ...
            try
            {
                mainCam.farClipPlane = renderDistINI*3;
